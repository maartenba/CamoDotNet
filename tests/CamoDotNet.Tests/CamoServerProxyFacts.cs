...
namespace CamoDotNet.Tests
{
        ...
        : CamoServerFactsBase
    {
        ...
        public async Task DoesNotProxyInvalidContentType()
        {
            using (var server = CreateServer())
            {
                HttpResponseMessage response = await server.CreateClient().GetAsync(GenerateSignedUrl("https://www.github.com"));
        ...
        public async Task DoesNotProxyInvalidUrl()
        {
            using (var server = CreateServer())
            {
                HttpResponseMessage response = await server.CreateClient().GetAsync(GenerateSignedUrl("https://www.github.com.thisdoesnotexist"));
